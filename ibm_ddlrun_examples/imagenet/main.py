...
parser.add_argument('--dist-backend', default='ddl', type=str,
...
parser.add_argument('--lms-size', default=-1, type=int,
                    help='size (in KB)')
parser.add_argument('--lms-limit', default=0, type=int,
help='limit (in MB)')
...
def main_worker(args):
    ...
    if (torch.cuda.get_enabled_lms()):
        if (args.lms_size >= 0):
            torch.cuda.set_size_lms(args.lms_size * 1024)
        if (args.lms_limit > 0):
            torch.cuda.set_limit_lms(args.lms_limit * 1024 * 1024)
        print('[LMS=On size=' + str(torch.cuda.get_size_lms()) + ' limit=' + str(torch.cuda.get_limit_lms()) + ']')
